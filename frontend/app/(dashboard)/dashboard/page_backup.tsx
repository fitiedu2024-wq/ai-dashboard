                ...
                <div key={job.id} className="p-6 bg-white/5 border border-white/10 rounded-2xl hover:bg-white/10 transition">
                  ...
                  {selectedJob?.id === job.id && job.results && (
                    <div className="mt-6 space-y-6">
                      ...
                      {/* Trackers */}
                      {job.results.trackers && job.results.trackers.active_trackers.length > 0 && (
                        <div className="p-4 bg-white/5 rounded-xl">
                          <h5 className="text-lg font-bold text-white mb-3">📊 Marketing Trackers ({job.results.trackers.active_count})</h5>
                          <div className="grid grid-cols-2 md:grid-cols-3 gap-2">
                            {job.results.trackers.active_trackers.map((tracker: string) => (
                              <span key={tracker} className="px-3 py-2 bg-green-500/20 text-green-200 rounded-lg text-sm text-center">
                                {tracker.replace(/_/g, ' ')}
                              </span>
                            ))}
                          </div>
