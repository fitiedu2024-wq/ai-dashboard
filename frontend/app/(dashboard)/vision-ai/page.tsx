...
export default function VisionAI() {
  ...
  const isSuccess = results?.success || results?.data?.success;
  const method = results?.method || results?.data?.method;
  const note = results?.note || results?.data?.note;
        ...
        {isSuccess && (
          <div className="space-y-6">
            ...
            <div className="glass rounded-2xl p-8 border border-white/20">
              ...
              {/* Image Properties */}
              {analysisData?.image_properties && Object.keys(analysisData.image_properties).length > 0 && (
                <div className="mb-6">
                  <h4 className="font-bold text-lg text-white mb-4">📐 Image Properties</h4>
                  <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                    <div className="bg-white/5 p-4 rounded-xl text-center">
                      <div className="text-2xl font-bold text-blue-400">
                        {analysisData.image_properties.width} × {analysisData.image_properties.height}
                      </div>
                      <div className="text-xs text-gray-400 mt-1">Dimensions</div>
                    </div>
                    <div className="bg-white/5 p-4 rounded-xl text-center">
                      <div className="text-2xl font-bold text-purple-400">
                        {analysisData.image_properties.aspect_ratio}
                      </div>
                      <div className="text-xs text-gray-400 mt-1">Aspect Ratio</div>
                    </div>
                    <div className="bg-white/5 p-4 rounded-xl text-center">
                      <div className="text-2xl font-bold text-green-400">
                        {analysisData.image_properties.format || 'Unknown'}
                      </div>
                      <div className="text-xs text-gray-400 mt-1">Format</div>
                    </div>
                    <div className="bg-white/5 p-4 rounded-xl text-center">
                      <div className="text-2xl font-bold text-cyan-400">
                        {analysisData.image_properties.is_banner ? '🖼️ Banner' :
                         analysisData.image_properties.is_square ? '⬜ Square' : '📷 Standard'}
                      </div>
              ...
              {analysisData?.colors && analysisData.colors.length > 0 && (
                <div className="mb-6">
                  ...
                  <div className="flex flex-wrap gap-3">
                    {analysisData.colors.map((color: any, idx: number) => (
              ...
              {analysisData?.labels?.length > 0 && (
                <div className="mb-6">
                  ...
                  <div className="flex flex-wrap gap-2">
                    {analysisData.labels.map((label: any, idx: number) => (
                      <span
                        key={idx}
              ...
              <div className="bg-purple-500/10 p-6 rounded-xl border border-purple-500/20 mt-6">
                ...
                <div className="space-y-2 text-sm text-gray-300">
                  <div>• <strong>Brand Visibility:</strong> {analysisData?.logos?.length || 0} brand(s) detected in this image</div>
                  <div>• <strong>Detection Quality:</strong> {analysisData?.logos?.length > 0 ? 'High confidence detections' : 'No clear logos detected'}</div>
                  <div>• <strong>Analysis Method:</strong> {method === 'local_analysis' ? '🖥️ Local PIL Analysis' : '🔗 URL Pattern Analysis'}</div>
                  <div>• <strong>Recommendation:</strong> {analysisData?.logos?.length > 0 ? 'Monitor this brand presence for competitive insights' : 'Try a different image with clearer logos'}</div>
                </div>
              </div>

              <div className="mt-6 p-4 bg-blue-500/10 border border-blue-500/20 rounded-xl">
                <p className="text-sm text-blue-300">
                  💡 <strong>Note:</strong> {note || 'Enhanced with local image analysis using PIL/Pillow.'}
                </p>
              </div>
            </div>
          </div>
