...
export function exportSEOReportToPDF(yourSite: any, competitors: Record<string, any>, insights: string[], domain: string): void {
  const metricsHTML = `
    <div style="display: flex; flex-wrap: wrap; gap: 10px; margin: 20px 0;">
      <div class="metric">
        <div class="metric-value">${yourSite.avg_seo_score}</div>
        <div class="metric-label">SEO Score</div>
      </div>
      <div class="metric">
        <div class="metric-value">${yourSite.total_pages}</div>
        <div class="metric-label">Pages Analyzed</div>
      </div>
      <div class="metric">
        <div class="metric-value">${yourSite.avg_word_count}</div>
        <div class="metric-label">Avg Words/Page</div>
      </div>
      <div class="metric">
        <div class="metric-value">${yourSite.avg_alt_coverage}%</div>
        <div class="metric-label">Alt Coverage</div>
      </div>
      <div class="metric">
        <div class="metric-value">${yourSite.schema_coverage}%</div>
    ...
    <table>
      ...
      <tbody>
        ${Object.entries(competitors).map(([compDomain, data]: [string, any]) => `
          <tr>
            <td>${compDomain}</td>
            <td>${data.avg_seo_score}</td>
            <td>${data.total_pages}</td>
            <td>${data.avg_word_count}</td>
            <td>${data.avg_alt_coverage}%</td>
            <td>${data.schema_coverage}%</td>
          </tr>
        `).join('')}
      </tbody>
    </table>
  ...
  const insightsHTML = insights.length > 0 ? `
    <h2>Key Insights</h2>
    ${insights.map(insight => `<div class="recommendation">${insight}</div>`).join('')}
  ` : '';

  const issuesHTML = yourSite.issues?.length > 0 ? `
    <h2>Issues Found</h2>
    ${yourSite.issues.map((issue: string) => `<div class="issue">${issue}</div>`).join('')}
  ` : '';

  const recommendationsHTML = yourSite.recommendations?.length > 0 ? `
    <h2>Recommendations</h2>
    ${yourSite.recommendations.map((rec: string) => `<div class="recommendation">${rec}</div>`).join('')}
  ` : '';

  const content = `
    <h2>Your Site: ${domain}</h2>
    ${metricsHTML}
    ${competitorsHTML}
    ${insightsHTML}
    ${issuesHTML}
  ...
  exportToPDF(`SEO Analysis Report - ${domain}`, content, `seo-report-${domain}`);
}
